...
class ProductController extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->lang->load('product', $this->config->item("language"));
	}

	public function index() {
		$products = $this->product->get_products_list();
		$permission = $this->user->hasAuthority('product');
		if (empty($permission))
	...
	public function add_product() {
		...
		if ($this->form_validation->run() != FALSE) {
			...
			$new_product['type'] = $this->input->post('type');
			...
			if ($msg === 'already_exist') {
				...
				$data['product_permis'] = $this->user->hasAuthority('product');
