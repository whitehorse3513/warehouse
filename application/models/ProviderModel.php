...
class ProviderModel extends CI_model
{
    ...
    function get_provider_list($start = 0, $limit = 0, $keyword = '') {
        ...
        $query = $this->db->query('select a.*,country.countryname,country.id as countryid from provider as a left join country on country.id = a.country limit '.$start.', '.$limit);
        return $query->result_array();
    }
    function get_list() {
        $query = $this->db->query('select a.*,country.countryname,country.id as countryid from provider as a left join country on country.id = a.country');
        return $query->result_array();
    }
    function search_result($key_word) {
        ...
        $this->db->or_like('state', $key_word);
