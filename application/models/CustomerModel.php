...
class CustomerModel extends CI_model
{
    ...
    function get_customer_list($start = 0, $limit = 0, $keyword = '') {
        ...
        $query = $this->db->query('select a.*,country.countryname,country.id as countryid from customer as a left join country on country.id = a.country limit '.$start.', '.$limit);
        return $query->result_array();

    }
    function get_list() {

        $query = $this->db->query('select a.*,country.countryname,country.id as countryid from customer as a left join country on country.id = a.country');
        return $query->result_array();
    }
